...
for (let i = 0; i < faq_quest.length; i++) {
    ...
    faq_quest[i].onclick = () => {
        ...
        fa[i+1].classList.add('animated');
        if(flag[i]){
            console.log(flag[i]);
            fa[i+1].classList.remove('fa-angle-up'); // i+1 потому что одно fa уже было
            fa[i+1].classList.add('fa-angle-down');
            fa[i+1].classList.toggle('rotateIn'); 
            setTimeout(() => {
            fa[i+1].classList.toggle('rotateIn');
        }, 600);
         ...
         {
            fa[i+1].classList.remove('fa-angle-down');
            fa[i+1].classList.add('fa-angle-up');
            fa[i+1].classList.toggle('rotateIn'); 
            setTimeout(() => {
                fa[i+1].classList.toggle('rotateIn');
            }, 600);
